...
public:
    ...
    PolarizedPlastic(const Properties &props) : Base(props) {
        ...
        if (props.has_property("alpha")) {
            m_alpha_u = props.texture<Texture>("alpha");
            m_alpha_v = props.texture<Texture>("alpha");
        } else {
    ...
    void parameters_changed(const std::vector<std::string> &/*keys*/ = {}) override {
        ...
        Float alpha_v_mean = m_alpha_v->mean();
        Float alpha_u_mean = m_alpha_u->mean();

        dr::make_opaque(m_eta, alpha_u_mean, alpha_v_mean, m_specular_sampling_weight);
    }
